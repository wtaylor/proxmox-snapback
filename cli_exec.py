...
def _raw_execute_snapshot_mount(zfs_dataset_snapshot_uid, mount_root):
    return subprocess.run(['mount', '-t', 'zfs', f'{zfs_dataset_snapshot_uid}', f'{mount_root}'], text=True, capture_output=True)
